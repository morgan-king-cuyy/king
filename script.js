// --- 1. SETUP CANVAS & EVENT INPUT (MOUSE + TOUCH MOBILE) ---
...
canvas.style.zIndex = "0"; // Di belakang UI
canvas.style.backgroundColor = "black";
canvas.style.pointerEvents = "none"; // Supaya menu/tombol bisa diklik
...
// Fitur Layar Sentuh untuk HP Android
...
// --- 2. CLASS & LOGIKA ANIMASI REPTIL (ORIGINAL CODE) ---
var segmentCount = 0;
...
// Menjalankan Lizard dengan Jumlah Kaki Acak
...
function playMusic() {
    if (bgMusic) {
        ...
        }).catch(err => console.log("Autoplay tercegah browser:", err));
    }
}
...
// Play Musik saat Klik / Touch Pertama Kali
function autoPlayFirstTouch() {
    if (!isPlaying) {
        playMusic();
    }
    window.removeEventListener('click', autoPlayFirstTouch);
    window.removeEventListener('touchstart', autoPlayFirstTouch);
}

window.addEventListener('click', autoPlayFirstTouch);
window.addEventListener('touchstart', autoPlayFirstTouch);

// Toggle Play/Pause dari Tombol Musik Bulat
if (musicBtn) {
